...
TextView::TextView(axl::gl::Context* ptr_context,
	...
	text_horizontal_text_alignment(HAL_CENTER),
	text_vertical_text_alignment(VAL_CENTER),
	text_text(ptr_context)
{}
TextView::~TextView()
{
	this->destroy();
...
void TextView::setContext(axl::gl::Context* ptr_context)
{
	...
	text_text.setContext(ptr_context);
}
bool TextView::isValid() const
{
	return axl::gl::gfx::ui::Element::isValid() && text_text.isValid();
}
const axl::util::WString& TextView::getLabel() const
{
	return text_text.getText();
}
bool TextView::setLabel(const axl::util::WString& label)
{
	if(!text_text.setText(label)) return false;
...
{
	return text_text.setTextProgram(text_program);
}
...
{
	text_horizontal_text_alignment = horizontal_text_alignment;
	this->onAlignmentChange();
}
void TextView::setVerticalAlignment(VerticalTextAlignment vertical_text_alignment)
{
	text_vertical_text_alignment = vertical_text_alignment;
	this->onAlignmentChange();
}
void TextView::setForegroundColor(const axl::math::Vec4f& foreground_color)
{
	...
	text_text.setColor(this->component_foreground_color);
}

TextView::HorizontalTextAlignment TextView::getHorizontalAlignment() const
{
	return text_horizontal_text_alignment;
}
TextView::VerticalTextAlignment TextView::getVerticalAlignment() const
{
	return text_vertical_text_alignment;
}

bool TextView::iCreate()
{
	if(!axl::gl::gfx::ui::Element::iCreate() || !text_text.create()) return false;
	return true;
}
bool TextView::iDestroy()
{
	return axl::gl::gfx::ui::Element::iDestroy() && text_text.destroy();
}
bool TextView::iRender(axl::gl::camera::Camera3Df* camera)
{
	return text_text.render(camera);
}
axl::math::Vec2i TextView::getContentSize() const
{
	const axl::math::Vec2f& text_box = text_text.getBox();
...
void TextView::onAlignmentChange()
{
	...
	// 	return;
	axl::math::Vec2f text_size = text_text.getBox();
	axl::math::Vec3f position(0.f,0.f,0.f);
	switch(text_horizontal_text_alignment)
	{
		case HAL_CENTER:
			text_text.setHorizontalAlignment(axl::gl::gfx::Text::TAL_HORIZONTAL_CENTER);
			position.x = (float)this->component_size.x / 2.f;
			break;
		case HAL_LEFT:
			text_text.setHorizontalAlignment(axl::gl::gfx::Text::TAL_LEFT);
			position.x = component_padding.x;
			break;
		case HAL_RIGHT:
			text_text.setHorizontalAlignment(axl::gl::gfx::Text::TAL_RIGHT);
			position.x = (float)this->component_size.x - component_padding.z;
			break;
	}
	switch(text_vertical_text_alignment)
	{
		case VAL_CENTER:
			text_text.setVerticalAlignment(axl::gl::gfx::Text::TAL_VERTICAL_CENTER);
			position.y = (float)this->component_size.y / 2.f;
			break;
		case VAL_BOTTOM:
			text_text.setVerticalAlignment(axl::gl::gfx::Text::TAL_BOTTOM);
			position.y = component_padding.y;
			break;
		case VAL_TOP:
			text_text.setVerticalAlignment(axl::gl::gfx::Text::TAL_TOP);
			position.y = (float)this->component_size.y - component_padding.w;
			break;
	}
	text_text.text_transform.setPosition(position);
}
