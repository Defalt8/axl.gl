		...
		bool onCreate(bool recreating)
		{
			...
			if(this->main_context.makeCurrent())
			{
				...
				if(vertex_array != 0 && vertex_buffer != 0)
				{
					...
					// float rsize_x = (float)font_size.x / texture_size.x;
					// float rsize_y = (float)font_size.y / texture_size.y;
					GL::GLfloat tminx = char_uv.x;//(float)char_pos.x / texture_size.x;
					GL::GLfloat tmaxx = char_uv.z;//tminx + rsize_x;
					GL::GLfloat tminy = char_uv.y;//(float)char_pos.y / texture_size.y;
					GL::GLfloat tmaxy = char_uv.w;//tminy + rsize_y;
