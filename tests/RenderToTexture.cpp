...
class GameView : public axl::gl::View
{
	public:
		...
		axl::gl::gfx::Texture2D texture2d_color, texture2d_depth;
		axl::gl::gfx::Texture2D texture2d_color0, texture2d_depth0;
	...
	public:
		GameView(const axl::util::WString& _title, const axl::math::Vec2i& _position, const axl::math::Vec2i& _size, const axl::gl::Cursor& _cursor = axl::gl::DefaultCursor) :
			...
			texture2d_color(&this->main_context),
			texture2d_depth(&this->main_context),
			samples(0),
			offscreen(true),
			theta(0.0f),
		...
		bool onCreate(bool recreating)
		{
			...
			Assert(!this->texture2d_color.isValid());
			Assert(this->texture2d_color.create());
			Assert(this->texture2d_color.isValid());

			Assert(!this->texture2d_depth.isValid());
			Assert(this->texture2d_depth.create());
			Assert(this->texture2d_depth.isValid());
...
int main(int argc, char* argv[])
{
	...
	puts("----------------------------------------");
	{
		...
		GameView view(L"axl.gl.RenderToTexture", position, size);
